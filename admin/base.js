...
var connString = 'http://92.62.141.163/api/v1/sync';
...
app.config(['$routeProvider', '$locationProvider', function($routeProvider, $locationProvider) {
    ...
    $routeProvider.when('/sections', {
        templateUrl: tmpPath + 'tpl/sections.tpl.html',
        controller: 'SectionsController'
    });

    $routeProvider.when('/designers', {
        templateUrl: tmpPath + 'tpl/designers.tpl.html',
    ...
    .controller('DiscountController', ['$scope', '$http', '$location', '$routeParams', '$timeout', function($scope, $http, $location, $routeParams, $timeout) {
        ...
        $scope.update = function(tableData){
            $http({
                url: connString + 'discount/' + tableData.code,
                method: 'POST',
                json: true,
                data: {
                    id: tableData.id,
                    discount: tableData.discount,
                    voucher: tableData.code,
                }
            }).success(function() {
...
/*            for (var i = 0; i < data.length; i++) {
        ...
        $scope.update = function(tableData){
            $http({
                url: connString + 'newsUpdate/' + tableData.id,
                method: 'POST',
                json: true,
                data: {
                    id: tableData.id,
    ...
    .controller('CategoriesController', ['$scope', '$http', '$location', '$routeParams', '$timeout', function($scope, $http, $location, $routeParams, $timeout) {
        ...
        $scope.enableCategory = function(id, state, name) {
            console.log(id);
            console.log(state)
            $http({
                url: connString + 'enableCategory/' + id,
                method: 'POST',
                json: true,
                data: {
                    visible: state,
                ...
                console.log('changed category state for id ' + id)
            });
        };

    }])

    .controller('SectionsController', ['$scope', '$http', '$location', '$routeParams', '$timeout', function($scope, $http, $location, $routeParams, $timeout) {
        //$scope.name = $routeParams.name;
        $scope.raw = {};
        $http({ url: connString + 'sections', method: 'GET' }).success(function(data) {
            $scope.brands = data;
            $scope.currentPage = 1; //current page
            $scope.entryLimit = 50; //max no of items to display in a page
            $scope.filteredItems = $scope.brands.length; //Initially for no filter
        ...
        $scope.enableSection = function(id, state) {
            console.log(id);
            console.log(state)
            $http({
                url: connString + 'enableSection/' + id,
                method: 'POST',
                json: true,
                data: {
                    visible: state
                ...
                console.log('changed section state for id ' + id)
            });
        };
    }])

    .controller('DesignController', ['$scope', '$http', '$location', '$routeParams', '$timeout', function($scope, $http, $location, $routeParams, $timeout) {
        //$scope.name = $routeParams.name;
        $scope.raw = {};
        $http({ url: connString + 'designers', method: 'GET' }).success(function(data) {
            $scope.brands = data;
            $scope.currentPage = 1; //current page
            $scope.entryLimit = 50; //max no of items to display in a page
            $scope.filteredItems = $scope.brands.length; //Initially for no filter
        ...
        $scope.enableDesigner = function(id, state) {
            console.log(id);
            console.log(state)
            $http({
                url: connString + 'enableDesigner/' + id,
                method: 'POST',
                json: true,
                data: {
                    visible: state
                ...
                console.log('changed designer state for id ' + id)
            });
        };
    }])
    ...
    .controller('SubCategoriesController', ['$scope', '$http', '$location', '$routeParams', '$timeout', function($scope, $http, $location, $routeParams, $timeout) {
        ...
        $scope.enableSubCategory = function(id, state, name) {
            console.log(id);
            console.log(state)
            $http({
                url: connString + 'enableSubCategory/' + id,
                method: 'POST',
                json: true,
                data: {
                    visible: state,
                ...
                console.log('changed sub category state for id ' + id)
            });
        };
   ...
   // starts the actions for shop banner     
    ...
    .controller('DesignerController', ['$scope', '$http', '$location', '$routeParams', '$timeout', function($scope, $http, $location, $routeParams, $timeout) {
        ...
        $scope.proNameLocale = $scope.empty;
        $scope.descriptionLocale =  {
            ru: "",
            en: "",
            ua: "",
            de: "",
        ...
        $scope.avatarPhotoFileChanged = function(ele) {
            getPromiseForAvatar(ele.files[0]);
        }

        function getPromiseForAvatar(file){
            return new Promise(function(resolve, reject){
                var reader = new FileReader();
                reader.onload = function(evt){
                    $scope.$apply(() => {
                        $scope.avatarUrl = evt.target.result;
                    });
                };
                reader.onerror = function(error) {
                    reject(error);
        ...
        function otherInfoUpload(){
            if( $scope.avatarUrl === "" ) {
                alert("Please select a avartar image");
                return;
            }
            let formData = new FormData();
            formData.append('image_file', $scope.avatarUrl);
            formData.append('tag', $scope.solutionType);
            formData.append('projectName', $scope.proName);
            formData.append('solutionNumber', $scope.solNumber === "" ? 0 : parseInt($scope.solNumber));
            formData.append('currency', $scope.currency);
            formData.append('description', $scope.description);
            formData.append('designerName', $scope.designerName);
            formData.append('pNameLocale', JSON.stringify($scope.proNameLocale));
            formData.append('descLocale', JSON.stringify($scope.descriptionLocale));

            var request = new XMLHttpRequest();
            request.open("POST", connString + 'designer/otherinfo');
            request.onload = function () {
                if (request.readyState === request.DONE) {
                    if (request.status === 200) {
                        var response = JSON.parse(request.responseText);
        ...
        $scope.handleDesignerName = function(txt){
            $scope.designerName = txt;
        }
        $scope.handleProName = function(txt){
            $scope.proName = txt;
        }
        $scope.handleSolutionNumber = function(txt){
            $scope.solNumber = txt;
        }
        $scope.handleCurrency = function(txt){
            $scope.currency = txt;
        }
        $scope.handleDesc = function(txt){
            $scope.description = txt;
        }

        $scope.handleProjectNameLocale = function(key, txt) {
            $scope.proNameLocale[`${key}`] = txt;
        }
        $scope.handleDescriptionLocale = function(key, txt) {
            $scope.descriptionLocale[`${key}`] = txt;
        }
    }])
